...
import { Trophy, Users, ChevronLeft, Search, Filter, Calendar, MapPin } from 'lucide-react';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '../ui/card';
import { Button } from '../ui/button';
import { Badge } from '../ui/badge';
import type { Language } from '../../App';
...
export function ContestsScreen({ language }: { language: Language }) {
  ...
  const loadContestDetail = (id: number) => {
    fetch(`http://localhost:3001/api/contests/${id}`)
      .then(res => res.json())
      .then(json => {
        if (json.success) setSelectedContest(json.data);
      })
      .catch(() => toast.error('Server error'));
  };

  const handleRegister = (id: number) => {
    fetch(`http://localhost:3001/api/contests/${id}/register`, { method: 'POST' })
      .then(res => res.json())
      .then(json => {
        if (json.success) {
          ...
          if (selectedContest?.id === id) setSelectedContest(json.data);
        } else {
  ...
  if (selectedContest) {
    ...
    return (
      <div className="p-6 max-w-7xl mx-auto">
        <Button variant="ghost" onClick={() => setSelectedContest(null)} className="mb-4">
          <ChevronLeft className="h-4 w-4 mr-2" /> {t.back}
        </Button>

        <Card className="mb-6">
          <CardContent>
            <h1 className="text-3xl mb-2">{c.title}</h1>
            <p className="text-gray-700 mb-4">{c.description}</p>

            <div className="space-y-2 text-sm text-gray-600">
              <p><Calendar className="inline-block h-4 w-4 mr-1" /> {c.startDate} → {c.endDate}</p>
              <p><MapPin className="inline-block h-4 w-4 mr-1" /> {c.location}</p>
              <p>🏫 {c.organizer}</p>
              <p>👥 {c.participants}/{c.maxParticipants} {t.participants}</p>
              {c.rules && <p>🧾 Rules: {c.rules.join(', ')}</p>}
            </div>

            <Button
              className={`mt-4 w-full ${registered.includes(c.id) ? 'bg-green-600 text-white' : ''}`}
              disabled={c.status === 'closed' || registered.includes(c.id)}
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredContests.map(c => (
          <Card key={c.id} className="cursor-pointer hover:shadow-md" onClick={() => loadContestDetail(c.id)}>
            <CardHeader>
              <div className="flex justify-between items-start">
                <Trophy className={`h-8 w-8 ${c.type === 'academic' ? 'text-purple-600' : 'text-indigo-600'}`} />
                <Badge variant={c.status === 'open' ? 'default' : 'secondary'}>
            ...
            <CardContent>
              <div className="flex justify-between text-sm text-gray-600">
                <span><Calendar className="inline-block h-4 w-4 mr-1" /> {c.startDate}</span>
                <span>{c.participants} {t.participants}</span>
              </div>
              <Button
                className={`mt-2 w-full ${registered.includes(c.id) ? 'bg-green-600 text-white' : ''}`}
                disabled={c.status === 'closed' || registered.includes(c.id)}
                onClick={e => { e.stopPropagation(); handleRegister(c.id); }}
              >
                {registered.includes(c.id) ? t.registered : t.register}
              </Button>
            </CardContent>
