...
export function QAScreen({ language, user }: { language: Language , user: User}) {
  const [questions, setQuestions] = useState<Question[]>([]);
  const [selectedQuestionId, setSelectedQuestionId] = useState<number | null>(null);
  ...
  // ============================
  useEffect(() => {
    fetch('http://localhost:3001/api/getquestions')
      .then(res => res.json())
      .then(data => setQuestions(data))
      .catch(() => toast.error("Cannot load questions"));
  }, []);
  ...
  const handlePostQuestion = async () => {
    ...
    const body = {
      ...
      topic: newQuestionTopic,
      content: newQuestionContent,
      author: user.bknetId,   // <-- FIXED
    };

    const res = await fetch("http://localhost:3001/api/addquestion", {
      method: "POST",
  ...
  const handlePostAnswer = async () => {
    ...
    const res = await fetch(`http://localhost:3001/api/questions/${selectedQuestionId}/answers`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        content: newAnswerContent,
